...
real*8 :: noise(nnoise),xsave(npoichk,0:ndh),xexact(0:nnoise),noise2(nnoise),noise3(nnoise)
...
idum3 = -41 - 1000*rank   ! Different seed for each rank
dh0 = 1.d0/npoichk
ndh0 = 1 !  ndh-7
a=1.5d0 ! 2.d0
pi = 4.d0*datan(1.d0)
...
! equivalent noise
      do inner=1,ninner
         ...
         w3 = 0.d0
         do iw = 1,nskip
            w1 = w1 + noise(iw+(ipoints-1)*ninner*nskip+(inner-1)*nskip)
            w2 = w2 + noise2(iw+(ipoints-1)*ninner*nskip+(inner-1)*nskip)
            w3 = w3 + noise2(iw+(ipoints-1)*ninner*nskip+(inner-1)*nskip)
         enddo
!         wtot = wtot + w0
         z1 = w1 * d1
