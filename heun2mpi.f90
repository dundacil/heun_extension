...
integer, parameter :: nd=500, nbin=2*nd+1, nnd=20, nalgo=17
...
ninner = 10000
nave = 5000 ! 10000
jmax = nint(1.d0/dh)   !internal loop
...
if (rank == root) then
rho_global=rho_global/size/ninner/nave/dx
pi = 4.d0*datan(1.d0)
residui = 0.d0
resiass = 0.d0
aa = 1.d0/(2.d0*d0)
...
do ialgo = 1,nito
residui(ialgo) = residui(ialgo) + dabs(theory-rho_global(i+nd+1,ialgo))
  if(theory.ge.anorm*1.d-4) resiass(ialgo) = resiass(ialgo) + dabs(1.d0-rho_global(i+nd+1,ialgo)/theory)
enddo
...
if(id.eq.1) then
fmt_string=""
write(fmt_string, '(A,I0,A)') "(A,1x,A,1x,A,1x,", nalgo, "(A4,1x))"
write(80,fmt_string) "descriptor","dh","d0",(algo(ialgo),ialgo=1,nalgo)
write(81,fmt_string) "descriptor","dh","d0",(algo(ialgo),ialgo=1,nalgo)
endif
write(80,*) dh,d0,(dx*residui(ialgo),ialgo=1,nalgo)
write(81,*) dh,d0,(dx*resiass(ialgo),ialgo=1,nalgo)
